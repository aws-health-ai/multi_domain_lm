...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        self.ulmfit = args.use_ulmfit
        self.inv_fisher = args.invert_fisher

        self.decay_rate_lrc = args.decay_rate_lrc
    ...
    def freeze_old_model(self):
        """
        Used to generate a copy of the model (frozen) for EWC regularizer.
        """
        self._model_old = deepcopy(self.model)

        for param in self._model_old.parameters():
            ...
            for n, p in self.model.named_parameters():
                if p.requires_grad:
                    ...
                    if param_name_split[2] == 'lm_head':  # last layer
                        ...
                        multiplier = self.decay_rate_lrc ** -(self.num_layers - layer)
                    else:  # first layer
                        multiplier = self.decay_rate_lrc ** -(self.num_layers + 1)
    ...
    def get_fisher_iterator(self, epoch, combine=True):
        """Return an EpochBatchIterator over the Fisher initialization dataset."""
        print('| loading train data for epoch {}'.format(epoch))
        self.task.load_dataset(self.args.train_subset_tem, epoch=epoch, combine=combine)
        return self.task.get_batch_iterator(
            dataset=self.task.dataset(self.args.train_subset_tem),
            max_tokens=self.args.max_tokens,
            max_sentences=self.args.max_sentences,
            max_positions=utils.resolve_max_positions(
                self.task.max_positions(),
    ...
    def fisher_step(self, samples, dummy_batch=False, raise_oom=False):
        """Do forward, backward Fisher init."""
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.fisher_step(
                        sample, self.model, self.fisher, self.criterion, total_len
                    )
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self._model_old, self.fisher, self.criterion, self.optimizer,
                        ignore_grad
                    )
